...
export function TitleDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {title.tags.genre?.map((g) => (
              <span key={g} className="pop-badge bg-accent-500 px-3 py-1 text-xs text-[var(--on-accent)]">
                {g}
              </span>
            ))}
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {title.platforms.map((p) => (
              <span
                key={p}
                className="pop-badge bg-[var(--bg-elevated)] px-3 py-1 text-xs text-[var(--text)]"
              >
                {p}
              </span>
            ))}
          ...
          <div className="mt-4 flex items-center gap-2 text-sm">
            <span className="font-black uppercase">Community rating:</span>
          ...
          {isAuthenticated && (
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <button
                ...
                className="pop-pressable bg-amber-400 px-4 py-2.5 text-sm font-black uppercase text-[var(--ink)]"
              >
                ★ Super-like
              </button>
              <button
                onClick={() => handleAction("pass")}
                className="pop-pressable bg-[var(--bg-elevated)] px-4 py-2.5 text-sm font-black uppercase"
              >
                Not interested
              </button>
            </div>
          )}
          {status && <p className="mt-2 text-sm font-bold text-accent-600">{status}</p>}
        </div>
      </div>
      ...
      {isAuthenticated && (
        <div className="pop-panel mt-10 bg-[var(--bg-elevated)] p-6">
          ...
          <button
            onClick={handleRate}
            disabled={myRating < 1}
            className="pop-pressable mt-3 bg-accent-500 px-4 py-2.5 text-sm font-black uppercase text-[var(--on-accent)] disabled:opacity-40"
          >
            Submit rating
          </button>
        </div>
      ...
      <div className="mt-10">
        ...
        {reviews.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {reviews.map((r) => (
              <div key={r.id} className="pop-panel p-4">
                <div className="flex items-center justify-between">
                  <span className="font-black uppercase">{r.displayName}</span>
      ...
      <Link to="/swipe" className="mt-10 inline-block text-sm font-bold text-accent-600 hover:underline">
        ← Back to swipe deck
      </Link>
    </div>
  );
