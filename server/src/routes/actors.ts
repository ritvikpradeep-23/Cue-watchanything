import { Router } from "express";
import { prisma } from "../lib/prisma";
import { toApiTitle } from "../lib/titles";
import { getPopularityScores } from "../lib/popularity";

export const actorsRouter = Router();

const PAGE_SIZE = 24;
const TOP_HITS_COUNT = 8;

function toApiActor(row: { id: string; name: string; photoUrl: string | null; industry: string; bio: string | null }) {
  return {
    id: row.id,
    name: row.name,
    photoUrl: row.photoUrl,
    industry: JSON.parse(row.industry) as string[],
    bio: row.bio,
  };
}

/** GET /api/actors?industry=&cursor= — paginated grid, optional industry filter. `industry`
 * matches against the actor's own industry array, same semantics as the title Browse filter. */
actorsRouter.get("/", async (req, res) => {
  const industry = typeof req.query.industry === "string" && req.query.industry ? req.query.industry : null;
  const cursor = Math.max(0, Number(req.query.cursor) || 0);

  // industry is JSON-encoded text, not a native array column, so the industry filter is
  // applied in JS after a DB fetch — same tradeoff already accepted for Title's tag filters.
  const all = await prisma.actor.findMany({ orderBy: { name: "asc" } });
  const filtered = industry ? all.filter((a) => (JSON.parse(a.industry) as string[]).includes(industry)) : all;

  const page = filtered.slice(cursor, cursor + PAGE_SIZE);
  const nextCursor = cursor + PAGE_SIZE < filtered.length ? cursor + PAGE_SIZE : null;

  res.json({ actors: page.map(toApiActor), total: filtered.length, nextCursor });
});

/** GET /api/actors/:id?genre= — profile + filmography split into top-hits (by internal
 * popularity signal, capped) and everything else, optionally narrowed to titles in the given
 * genre. The genre filter option list itself comes from a separate lightweight query the
 * client already has via this same response's `availableGenres`. */
actorsRouter.get("/:id", async (req, res) => {
  const id = String(req.params.id);
  const actor = await prisma.actor.findUnique({ where: { id } });
  if (!actor) return res.status(404).json({ error: "Actor not found" });

  const links = await prisma.titleActor.findMany({
    where: { actorId: id },
    include: { title: true },
  });
  if (links.length === 0) {
    // A person with zero linked titles shouldn't have a browsable profile — see spec edge case.
    return res.status(404).json({ error: "Actor not found" });
  }

  const genre = typeof req.query.genre === "string" && req.query.genre ? req.query.genre : null;

  const allTitles = links.map((l) => l.title);
  const availableGenres = [
    ...new Set(allTitles.flatMap((t) => (JSON.parse(t.tags).genre as string[]) ?? [])),
  ].sort();

  const filteredTitles = genre
    ? allTitles.filter((t) => ((JSON.parse(t.tags).genre as string[]) ?? []).includes(genre))
    : allTitles;

  const popularity = await getPopularityScores();
  const ranked = [...filteredTitles].sort((a, b) => (popularity.get(b.id) ?? 0) - (popularity.get(a.id) ?? 0));

  const topHits = ranked.slice(0, TOP_HITS_COUNT);
  const topHitIds = new Set(topHits.map((t) => t.id));
  const restOfFilmography = ranked.filter((t) => !topHitIds.has(t.id));

  res.json({
    actor: toApiActor(actor),
    availableGenres,
    activeGenre: genre,
    topHits: topHits.map(toApiTitle),
    filmography: restOfFilmography.map(toApiTitle),
  });
});
